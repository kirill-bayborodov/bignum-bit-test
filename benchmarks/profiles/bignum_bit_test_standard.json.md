# How-to: `bignum_bit_test_standard.json`

## Назначение

`bignum_bit_test_standard.json` — компактная versioned matrix для функциональной проверки и регрессионного baseline операции `bignum_bit_test`. Manifest использует schema version `1`, которую читает C11-инструмент `bench_matrix` из pinned `benchmark-framework v1.0.0`.
...
libs/benchmark-framework/dist/tools/bench_matrix \
  --manifest benchmarks/profiles/bignum_bit_test_standard.json \
  --output benchmarks/reports/bignum_bit_test_standard_matrix.json \
...
libs/benchmark-framework/dist/tools/benchmark_stats \
  --input benchmarks/reports/bignum_bit_test_standard_matrix.json \
  --output benchmarks/reports/bignum_bit_test_standard_summary.json
```
