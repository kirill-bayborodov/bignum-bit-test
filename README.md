...
[![C/ASM CI](https://github.com/kirill-bayborodov/bignum-bit-test/actions/workflows/ci.yml/badge.svg)](https://github.com/kirill-bayborodov/bignum-bit-test/actions/workflows/ci.yml)
[![GitHub release](https://img.shields.io/github/v/release/kirill-bayborodov/bignum-bit-test?label=release)](https://github.com/kirill-bayborodov/bignum-bit-test/releases/latest)

`bignum-bit-test` is a standalone C11/x86-64 YASM module that queries one bit of a fixed-capacity unsigned `bignum_t`. The production path is an x86-64 YASM implementation conforming to the System V AMD64 ABI. The operation validates the object length and bit index, reads the selected word and bit, and writes an integer result without modifying the operand.

The module is a production component of the `bignum-lib` family and preserves the build, test, distribution, and benchmark conventions described here.
...
```bash
git clone --recurse-submodules https://github.com/kirill-bayborodov/bignum-bit-test.git
cd bignum-bit-test
```
...
- **Explicit API status:** the public API exposes `bignum_bit_test_status_t` rather than reusing a generic core status type.
- **Constant-time bit query:** word selection and intra-word masking use fixed arithmetic and one output write.
- **Bounds protection:** invalid indices and invalid normalized lengths return explicit errors without changing output.
...
The public API is declared in `include/bignum_bit_test.h`:

```c
typedef enum {
    BIGNUM_BIT_TEST_SUCCESS        =  0,
    BIGNUM_BIT_TEST_ERROR_NULL_ARG = -1,
    BIGNUM_BIT_TEST_ERROR_INDEX  = -2,
    BIGNUM_BIT_TEST_ERROR_LENGTH = -3
} bignum_bit_test_status_t;

bignum_bit_test_status_t bignum_bit_test(
    const bignum_t *num,
    size_t bit_index,
    int *bit_value);
```
...
|---|---|---|
| `num == NULL` | `BIGNUM_BIT_TEST_ERROR_NULL_ARG` | No `bignum_t` object is dereferenced |
| Valid bit index and normalized length | `BIGNUM_BIT_TEST_SUCCESS` | `*bit_value` receives the selected bit; `num` is unchanged |
| `bit_index >= BIGNUM_CAPACITY * 64` | `BIGNUM_BIT_TEST_ERROR_INDEX` | Output is unchanged |
| `num->len > BIGNUM_CAPACITY` | `BIGNUM_BIT_TEST_ERROR_LENGTH` | Output is unchanged |

The operation is reentrant and thread-safe for independent objects. It does not mutate `num`; concurrent writers still require external synchronization.
...
application_status_t query_value(bignum_t *value)
{
    int bit_value = -1;
    bignum_bit_test_status_t status = bignum_bit_test(value, 5U, &bit_value);

    return status == BIGNUM_BIT_TEST_SUCCESS
...
| File | Scope |
|---|---|
| `tests/test_bignum_bit_test.c` | Deterministic API, contract, and boundary tests |
| `tests/test_bignum_bit_test_extra.c` | Extended state, preservation, and boundary checks |
| `tests/test_bignum_bit_test_mt.c` | Concurrent independent-object checks |
| `tests/test_bignum_bit_test_runner.c` | Distribution integration smoke test |
| `tests/benchmark_adapter/test_bignum_bit_test_benchmark_adapter.c` | C11 transport mapping, validation, deterministic initialization, operation, and checksum tests |
...
```text
benchmarks/bench_bignum_bit_test.c
benchmarks/bench_bignum_bit_test_mt.c
```
...
```text
benchmark=bignum_bit_test_st ... elapsed_seconds=<seconds> ns_per_call=<nanoseconds>
Benchmark finished.
```

The MT runner uses `benchmark=bignum_bit_test_mt`. The trailing marker is the success condition checked by the Makefile; it must remain after the machine-readable line.

| Mode | Input pattern | Purpose |
|---|---|---|
| `all_zero` | Every input `bignum_t` is zero; bit index is zero | Measures the zero-value fast path |
| `all_nonzero` | Inputs are populated through `BIGNUM_CAPACITY` words with nonzero top words | Measures the normal nonzero bit-query path |
| `mixed` | Alternating zero and nonzero input rows | Measures a mixed workload and branch behavior |

### Single-thread CLI

```text
bin/bench_bignum_bit_test \
  [--data-mode all_zero|all_nonzero|mixed] \
  [--input-kind zero|nonzero|mixed] \
  [--operation-kind bit-zero|bit-word|bit-test|bit-random|bit-mixed] \
...
| `BENCH_OPERATION_KIND` | `bit-random` | `bit-zero`, `bit-word`, `bit-test`, `bit-random`, or `bit-mixed` bignum operation transport value |
...
```bash
./bin/bench_bignum_bit_test \
  --input-kind nonzero --operation-kind bit-test --size-profile half \
...
`bench_matrix` invokes the pinned C11 `bench_matrix` and `benchmark_stats` tools directly, without Python or hardware PMU events. The default `benchmarks/profiles/bignum_bit_test_full.json` covers zero/nonzero bit-query paths, one/quarter/half/variable operand lengths, and safe near-capacity cases. `benchmarks/profiles/bignum_bit_test_standard.json` is the shorter bit-query-specific smoke manifest and can be selected through `BENCH_MATRIX_PROFILE`. Each JSON manifest has a companion how-to document with its exact vocabulary and baseline workflow.
